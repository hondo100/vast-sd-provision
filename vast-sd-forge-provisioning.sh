...
# Minimalinvasiv: robuste Downloads, keine kaputte Statistik, keine Parallel-Stats
# ==============================================================================
...
for entry in "${DOWNLOADS[@]}"; do
  ...
  case "$src" in
    HF_GATED:*)         download_hf_gated "$dest" "$name" "${src#HF_GATED:}" || VAST_WARN "Fehlgeschlagen: $name" ;;
    http://*|https://*) download_url "$dest" "$name" "$src" || VAST_WARN "Fehlgeschlagen: $name" ;;
    [0-9]*)             download_civitai "$dest" "$name" "$src" || VAST_WARN "Fehlgeschlagen: $name" ;;
...
if declare -p EXTENSIONS >/dev/null 2>&1 && [[ ${#EXTENSIONS[@]} -gt 0 ]]; then
  ...
  for repo in "${EXTENSIONS[@]}"; do
    ...
    if [[ -d "$dir" ]]; then
      ...
      git clone --depth=1 "$repo" && VAST_OK "Geklont: $dir" || VAST_WARN "Clone fehlgeschlagen: $repo"
    fi
  done
fi
